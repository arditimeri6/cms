...
if (!function_exists('getMenuItemsByLocale')) {
    /**
     * Converts items (json string) to array and return array.
     */
    function getMenuItemsByLocale($locale, $type = 'frontend')
    {
        $menu = new \App\Models\Menu\Menu();
        $menu = $menu->where('language_slug', $locale)->where('type', $type)->first();
...
if (!function_exists('getMenuItemsByLocaleAndById')) {
    /**
     * Converts items (json string) to array and return array.
     */
    function getMenuItemsByLocaleAndById($locale, $id)
    {
        $menu = new \App\Models\Menu\Menu();
        $menu = $menu->where('language_slug', $locale)->where('id', $id)->first();
...
if (!function_exists('inArrayRec')) {

    /**
     * @return bool
     */
    function inArrayRec($needle, $haystack, $strict = true)
    {
...
if (!function_exists('getBlogsByCategoryId')) {

    /**
     * @return bool
     */
    function getBlogsByCategoryId($id, $limit = 10, $orderBy = 'desc')
    {
        $blog_category = \App\Models\BlogCategories\BlogCategory::active()->find($id);

        if($blog_category) {
            $posts = $blog_category->blogs()
...
if (!function_exists('getBlogById')) {
    ...
    {
        $blog_post = \App\Models\Blogs\Blog::find($id);
